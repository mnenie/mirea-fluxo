...
import { useArchivePagination } from '~/composables/useArchivePagination'
import { useArchiveStore } from '~/stores/archive'

const archiveStore = useArchiveStore()
const { ordersArchive } = storeToRefs(archiveStore)

const { selectOrderPage, updateItemsPerPage, itemsPerPage, currentPage } = useArchivePagination()
...
<template>
  <div class="absolute w-full bottom-0 flex flex-row items-stretch justify-between mt-6">
    <Pagination v-slot="{ page }" :total="ordersArchive.length / (itemsPerPage / 10)" :sibling-count="1" show-edges :default-page="1" class="mb-0">
      <PaginationList v-slot="{ items }" class="flex items-center gap-1">
        ...
        <PaginationLast @click="selectOrderPage(ordersArchive.length / 50, itemsPerPage)" />
      </PaginationList>
    </Pagination>
